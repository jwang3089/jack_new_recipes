...
do_compile_append() {
    echo "do_compile_append(): jack1.bbappend ******************* " >> LOG2 
}

do_jack1() {
    echo "do_jack1(): jack1.bbappend...." >> LOG2
    echo "{THISDIR}/{PN}=${THISDIR}/${PN}" >> LOG2
    echo "The DELTA_KERNEL_DEFCONFIG_ctx0700_es1=${DELTA_KERNEL_DEFCONFIG_ctx0700_es1}" >> LOG2
    echo "The DELTA_KERNEL_DEFCONFIG=${DELTA_KERNEL_DEFCONFIG}" >> LOG2


    for deltacfg in ${DELTA_KERNEL_DEFCONFIG_ctx0700_es1}; do
        echo "deltacfg=${deltacfg}" >> LOG2
        cat ${deltacfg}  >> LOG2
        cat ${deltacfg}  >> defconfig 
    done

    for deltacfg in ${PREFERRED_VERSION}; do
        echo "Version=${deltacfg}" >> LOG2
        cat ${deltacfg}  >> LOG2
    done
